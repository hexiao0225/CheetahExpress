...
import { Phone, User, Clock, TrendingUp, CheckCircle2, XCircle, PhoneOff } from 'lucide-react'
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-600 mt-1">Real-time voice dispatch monitoring with AI sentiment analysis</p>
      </div>
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-gray-900 mt-1">{mockCallSessions.length}</p>
            </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-green-600 mt-1">
                {mockCallSessions.filter(s => s.outcome === 'accepted').length}
              </p>
            </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-red-600 mt-1">
                {mockCallSessions.filter(s => s.outcome === 'declined').length}
              </p>
            </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-cheetah-600 mt-1">
                {(mockCallSessions.reduce((acc, s) => acc + s.sentiment, 0) / mockCallSessions.length * 100).toFixed(0)}%
              </p>
            </div>
              ...
              <div className="flex items-start gap-4 flex-1">
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-2">
                    <h3 className="font-semibold text-gray-900">{session.driverName}</h3>
                    <span className={`px-2 py-1 rounded-full text-xs font-medium ${getOutcomeColor(session.outcome)}`}>
                      {session.outcome}
                    </span>
                  </div>
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    <div>
                      ...
                      <p className="font-medium text-gray-900">{session.orderId}</p>
                    </div>
                    <div>
                      <p className="text-gray-500">Driver ID</p>
                      <p className="font-medium text-gray-900">{session.driverId}</p>
                    </div>
                    <div>
                      <p className="text-gray-500">Duration</p>
                      <p className="font-medium text-gray-900">{session.duration}s</p>
                    </div>
                    <div>
                      <p className="text-gray-500">Sentiment Score</p>
                      <p className={`font-bold ${getSentimentColor(session.sentiment)}`}>
                  ...
                  {session.declineReason && (
                    <div className="mt-3 p-3 bg-red-50 rounded-lg">
                      ...
                      <p className="text-sm text-red-900">{session.declineReason}</p>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="h-2 bg-gray-200 rounded-full overflow-hidden">
                <div
                  className={`h-full transition-all ${
                    session.sentiment >= 0.7 ? 'bg-green-500' :
                    session.sentiment >= 0.5 ? 'bg-yellow-500' : 'bg-red-500'
                  }`}
