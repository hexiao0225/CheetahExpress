...
import { ArrowLeft, Package, User, Phone, CheckCircle2, XCircle, Clock } from 'lucide-react'
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <p className="font-medium text-gray-900">{audit?.order_details?.status || 'Processing'}</p>
          </div>
          <div>
            <p className="text-sm text-gray-500">Vehicle Type</p>
            <p className="font-medium text-gray-900">{audit?.order_details?.vehicle_type || 'N/A'}</p>
          </div>
        </div>
      </div>
          ...
          {audit?.compliance_checks?.map((check: any, index: number) => (
            <div key={index} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="font-medium text-gray-900">Driver {check.driver_id}</p>
                  <p className="text-sm text-gray-600">{check.reasons?.[0] || 'All checks passed'}</p>
                </div>
              </div>
            </div>
          ))}
      ...
      <motion.div
        ...
        className="bg-white rounded-xl shadow-sm border border-gray-200 p-6"
      >
        <h2 className="text-lg font-semibold text-gray-900 mb-4">Driver Rankings</h2>
        <div className="space-y-3">
          {audit?.rankings?.map((ranking: any, index: number) => (
            <div key={index} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="font-medium text-gray-900">Driver {ranking.driver_id}</p>
                  <p className="text-sm text-gray-600">Score: {ranking.score?.toFixed(2)}</p>
                </div>
              </div>
              <span className="text-sm text-gray-500">{ranking.eta_minutes?.toFixed(0)} min ETA</span>
            </div>
          ))}
        </div>
      </motion.div>
          ...
          {audit?.voice_calls?.map((call: any, index: number) => (
            <div key={index} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="font-medium text-gray-900">Driver {call.driver_id}</p>
                  <div className="flex items-center gap-4 mt-1">
                    <span className={`text-sm px-2 py-1 rounded ${
                      call.outcome === 'accepted' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'
                    }`}>
                      {call.outcome}
                    </span>
                    {call.sentiment && (
                  ...
                  {call.decline_reason && (
                    <p className="text-sm text-red-600 mt-1">{call.decline_reason}</p>
                  )}
                </div>
              </div>
            </div>
        ...
        >
          ...
          <div className="grid grid-cols-3 gap-4">
            ...
            <div>
              ...
              <p className="font-bold">{audit.assignments[0].distance_km?.toFixed(1)} km</p>
            </div>
            <div>
              <p className="text-sm opacity-90">Duration</p>
              <p className="font-bold">{(audit.assignments[0].duration_hours * 60).toFixed(0)} min</p>
            </div>
          </div>
        </motion.div>
      )}
